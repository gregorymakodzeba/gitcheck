...
export async function deployListingsFixture(): Promise<{
  ...
  const { CEO, CFO, SUPERADMIN, OWNER, BRIDGE, ANYONE, EVERYONE, LISTINGS_CONTRACT, tokens } =
  ...
  const estimatedGas = await listingsFactory.signer.estimateGas(
    listingsFactory.getDeployTransaction(tokens.address, listingFee, royaltyNumerator)
  );
  ...
  try {
    listings = await listingsFactory.deploy(tokens.address, listingFee, royaltyNumerator);
  ...
  await tokensContractAsCEO.revokeRole(await tokens.MARKETPLACE_ROLE(), LISTINGS_CONTRACT.address);
  await tokensContractAsCEO.grantRole(await tokens.MARKETPLACE_ROLE(), listings.address);
  await tokensContractAsCEO.grantRole(await tokens.BRIDGE_ROLE(), BRIDGE.address);

  // set listings contract roles
  let listingsContractAsCEO = new ethers.Contract(listings.address, listings.interface, CEO);
